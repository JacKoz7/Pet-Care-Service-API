...
describe("/api/client/bookings GET", () => {
  ...
  it("should return 401 if authorization header is missing or invalid", async () => {
    // ARRANGE
    const request = new NextRequest("http://localhost:3000/api/client/bookings", {
  ...
  it("should fetch and format client bookings successfully", async () => {
    // ARRANGE
    mockAdminAuth.verifyIdToken.mockResolvedValue({ uid: "test-uid" });
    ...
    expect(data.bookings[0].provider.city).toBe("Warsaw");
